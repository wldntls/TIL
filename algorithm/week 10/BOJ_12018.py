...
result=[] # 성준이의 마일리지를 담을 리스트

for k in range(n):
  p,l=map(int,input().split()) # 과목을 신청한 사람의 수 p, 과목의 수강 인원이 l
  mileage=list(map(int,input().split())) # 각 사람이 마일리지를 얼마나 넣었는지
  mileage.sort(reverse=True) # 내림차순으로 정렬

  if p<l: # 만약 과목을 신청한 사람이 수강 인원보다 적다면 (무조건 들을 수  있는 과목)
...
if m>0: # 이건 왜하는거지..? 모든 과목수를 다 들었는대도 m이 남으면 그때는 cnt 출력!, m = 0 때도 포함시켜야함
  print(cnt)